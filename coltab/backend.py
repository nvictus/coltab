...
class HDF5Store(Backend):
    ...
    DEFAULT_H5OPTS = {"compression": "gzip", "compression_opts": 6, "shuffle": True}
    ...
    def create_table(self, name, dtypes=None, data=None, init_size=0, max_size=None):
        ...
        if name in self.grp and isinstance(self.grp[name], h5py.Dataset):
            raise StorageError(
    ...
    def drop_table(self, name):
        ...
        grp = self.grp[name]
        if isinstance(grp, h5py.Group) and all(
            isinstance(k, h5py.Dataset) for k in grp.values()
        ):
            table_name = grp.name
            if table_name == "/":
...
class ZarrStore(Backend):
    ...
    def __init__(self, store_uri, consolidated=False, storage_options=None, **kwargs):
        ...
        self.storage_options = storage_options or self.DEFAULT_ZOPTS
    ...
    def create_table(self, name, dtypes=None, data=None, init_size=0):
        ...
        if name in self.grp and isinstance(self.grp[name], zarr.Array):
            raise StorageError(
    ...
    def drop_table(self, name):
        ...
        if isinstance(table_grp, zarr.Group) and all(
            isinstance(k, zarr.Array) for k in table_grp.values()
        ):
